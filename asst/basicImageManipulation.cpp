...
Image scaleNN(const Image &im, float factor) {
  ...
  int width = floor(im.width() * factor);
  int height = floor(im.height() * factor);

  Image scaleNNImage(width, height, im.channels());
  for (int x =0; x < width; x++){
    for(int y = 0; y < height; y++){
      for(int c = 0; c < im.channels(); c++){
        int inputx = round(x/factor);
        int inputy = round(y/factor);
        scaleNNImage(x, y, c) = im.smartAccessor(inputx, inputy, c);
...
float interpolateLin(const Image &im, float x, float y, int z, bool clamp) {
  ...
  //calculate weight
  float upperXWeight = abs(lowerx - x);
  float lowerXWeight = 1 - upperXWeight;
  float upperYWeight = abs(lowery - y);
  float lowerYWeight = 1 - upperYWeight;


  float bottomx = im.smartAccessor(lowerx, lowery,z,clamp) * lowerXWeight + im.smartAccessor(upperx, lowery,z, clamp) * upperXWeight;
  float topx = im.smartAccessor(lowerx, uppery,z, clamp) * lowerXWeight + im.smartAccessor(upperx, uppery,z, clamp) * upperXWeight;
  
  float interpolatedy = bottomx * lowerYWeight + topx * upperYWeight;
...
Image scaleLin(const Image &im, float factor) {
  ...
  Image scaleLinImage(width, height, im.channels());
  for (int x =0; x < width; x++){
    for(int y = 0; y < height; y++){
      for(int c = 0; c < im.channels(); c++){
        //scaling the input coords
        float inputx = x/factor;
        float inputy = y/factor;
        scaleLinImage(x, y, c) = interpolateLin(im, inputx, inputy, c);
      }
    }
  }

  return scaleLinImage;
...
Image rotate(const Image &im, float theta) {
  ...
  Image rotatedImage(im.width(), im.height(), im.channels());
  ...
  for (int x = 0; x < im.width(); x++){
    for (int y = 0; y < im.height(); y++){
      ...
      float horizontal = x - centerX;
      

      float hypotenuse = sqrt(pow(horizontal, 2.0) + pow(vertical, 2.0));
      
      

      //avoid division by 0 errors when dealing with right angles
      if (horizontal == 0){
        omega = (vertical > 0) ? M_PI/2 : 3*M_PI/2;
      }
      //theta = 0
      else if (vertical == 0) { 
        omega = (horizontal > 0) ? 0 : M_PI;
      }

      else {
        omega = (horizontal < 0) ? atan(vertical/horizontal) + M_PI : atan(vertical/horizontal);
      }

      float normalizedTheta = theta + omega;

      //rotated x's and y's
      float yPrime = sin(normalizedTheta) * hypotenuse + centerY;
      float xPrime = cos(normalizedTheta) * hypotenuse + centerX;
      
      
      for (int c = 0; c < im.channels(); c++){

        rotatedImage(x, y, c) = interpolateLin(im, xPrime, yPrime, c);
